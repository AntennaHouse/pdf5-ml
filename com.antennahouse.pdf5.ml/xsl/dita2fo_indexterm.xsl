...
>
    ...
    <!-- Key definition -->
    <xsl:key name="KEY_INDEX_START" match="*[contains-token(@class, 'topic/indexterm')][not(ancestor::*[contains-token(@class, 'topic/navtitle')])]" use="@start"/>
    <xsl:key name="KEY_INDEX_END"   match="*[contains-token(@class, 'topic/indexterm')][not(ancestor::*[contains-token(@class, 'topic/navtitle')])]" use="@end"  />
    ...
    <!-- 
     ...
     -->
    <xsl:template match="*[contains-token(@class, 'topic/indexterm')]" mode="MODE_PROCESS_INDEXTERM_MAIN">
        ...
        <!--xsl:message>CALLED match="*[contains-token(@class, 'topic/indexterm')]" mode="MODE_PROCESS_INDEXTERM_MAIN"</xsl:message>
        ...
        <xsl:choose>
            ...
            <xsl:when test="child::*[contains-token(@class, $CLASS_INDEXTERM)]">
                ...
                <xsl:apply-templates select="child::*[contains-token(@class, $CLASS_INDEXTERM)]" mode="MODE_PROCESS_INDEXTERM_MAIN">
            ...
            <xsl:when test="(child::*[contains-token(@class,$CLASS_INDEX_SEE)])
                     and not(child::*[contains-token(@class,$CLASS_INDEX_SEEALSO)])">
    ...
    <xsl:function name="ahf:findIndextermEnd" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="contains-token($prmRangeElem/@class, 'map/topicref')">
    ...
    <xsl:function name="ahf:findIndextermEndFromTopicRef" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="exists($topicContent)">
                <xsl:choose>
                    <xsl:when test="$topicContent/descendant-or-self::*[contains-token(@class, 'topic/topic')]/child::*[contains-token(@class, 'topic/prolog')]/child::*[contains-token(@class, 'topic/metadata')]//*[contains-token(@class, 'topic/indexterm')][@end=$prmEnd]">
                        <xsl:sequence select="true()"/>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:function name="ahf:findIndextermEndFromChildAndSibling" as="xs:boolean">
        ...
        <xsl:variable name="childValue" as="xs:boolean">
            <xsl:choose>
                <xsl:when test="$prmTopicRef/child::*[contains-token(@class, 'map/topiref')]">
                    <xsl:variable name="childTopicRef" select="$prmTopicRef/child::*[contains-token(@class, 'map/topiref')][1]"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$prmTopicRef/following-sibling::*[contains-token(@class, 'map/topicref')]">
                <xsl:variable name="siblingFollowingTopicRef" select="$prmTopicRef/following-sibling::*[contains-token(@class, 'map/topicref')][1]"/>
