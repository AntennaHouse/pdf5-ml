...
>
    ...
    <!-- 
     ...
     param:		prmDitaValChangeBarStyle (CSS like chage-bar style string)
     return:	element()?
     note:		
     -->
    <xsl:function name="ahf:genChangeBarBeginElem" as="element()?">
        <xsl:param name="prmElem" as="element()"/>
        <xsl:param name="prmDitaValChangeBarStyle" as="xs:string"/>
        <xsl:choose>
            <xsl:when test="string($prmDitaValChangeBarStyle)">
                <fo:change-bar-begin>
                    <xsl:for-each select="tokenize($prmDitaValChangeBarStyle, ';')">
                        <xsl:variable name="propDesc" select="normalize-space(string(.))"/>
                        <xsl:choose>
                            <xsl:when test="not(string($propDesc))"/>
                            <xsl:when test="contains($propDesc,':')">
